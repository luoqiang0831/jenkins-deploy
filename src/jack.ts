...
export abstract class JackBase implements QuickpickSet {
    ...
    let disposable = vscode.commands.registerCommand(command, async () => {
      try {
        ...
        vscode.window.showWarningMessage(
          `Could not display ${command} commands.`
        );
      }
    });
  ...
  public async showInformationMessage<T extends vscode.MessageItem>(
    message: string,
    ...items: T[]
  ): Promise<T | undefined> {
  ...
  public async showInformationModal<T extends vscode.MessageItem>(
    message: string,
    ...items: T[]
  ): Promise<T | undefined> {
    return vscode.window.showInformationMessage(
