...
export class JobJack extends JackBase {
  constructor() {
    ...
    ext.context.subscriptions.push(
      vscode.commands.registerCommand(
        "extension.jenkins-jack.job.addHistory",
        async (item?: any[] | JobTreeItem, items?: JobTreeItem[]) => {
          if (item instanceof JobTreeItem) {
            let jobs = !items
              ...
              : items
                  .filter(
                    (item: JobTreeItem) => JobTreeItemType.Job === item.type
                  )
                  .map((item: any) => item.job.fullName);

            ext.historyJack.addProject(jobs);
    ...
    ext.context.subscriptions.push(
      vscode.commands.registerCommand(
        "extension.jenkins-jack.job.build",
        async (item?: any | JobTreeItem, items?: JobTreeItem[]) => {
          let result: boolean | undefined = false;
          if (item instanceof JobTreeItem) {
            let jobs = !items
    ...
    return [
      {
        ...
        target: () =>
          vscode.commands.executeCommand("extension.jenkins-jack.job.disable"),
      },
      {
        label: "$(check)  Job: Enable",
        description: "Enables targeted jobs from the remote Jenkins.",
        target: () =>
          vscode.commands.executeCommand("extension.jenkins-jack.job.enable"),
      },
      // {
      //   label: "$(circle-slash)  Job: Delete",
      //   description: "Deletes targeted jobs from the remote Jenkins.",
      //   target: () =>
      //     vscode.commands.executeCommand("extension.jenkins-jack.job.delete"),
      // },
      {
        label: "$(browser)  Job: Open",
        description: "Opens the targeted jobs in the user's browser.",
        target: () =>
          vscode.commands.executeCommand("extension.jenkins-jack.job.open"),
      },
    ];
  }
  ...
  public async enable(jobs?: any[]) {
    jobs = jobs
      ...
      : await ext.connectionsManager.host.jobSelectionFlow(
          (j: any) => !j.buildable && j.type !== JobType.Folder,
          true
        );
    if (undefined === jobs) {
      return;
  ...
  public async build(jobs?: any[]) {
    ...
    return await this.actionOnJobs(jobs, async (job: any) => {
      // const values = await ext.connectionsManager.host.getGitBranch(
      //   job.fullName
      // );
